          ...
          <div>
            ...
            <div style="margin-top: 10px;">
              <input
                class="modal-input"
                type="text"
                value="${state.newFavoriteName}"
                @click="${e => e.stopPropagation()}"
                @input="${e => state.newFavoriteName = e.target.value}"
              />
            </div>
          </div>
        `,
...
function SearchSuggestions ({ suggestions, selectSuggestion, removeFavorite, renameFavorite }) {
  const isFavorite = s =>
