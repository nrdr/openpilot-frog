...
FrogPilotLongitudinalPanel::FrogPilotLongitudinalPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> longitudinalToggles {
    ...
    {"SpeedLimitFiller", tr("Speed Limit Filler Tool"), tr("Capture speed limit data from the dashboard, <b>Mapbox</b>, or <b>Navigate on openpilot</b> for use with <b>OpenStreetMap</b>. Export data via <b>Fleet Manager</b> in <b>Tools</b> using the <b>Load Custom Data</b> option and submit it to <b>https://nerftool.glitch.me</b>."), ""},
  ...
  for (const auto &[param, title, desc, icon] : longitudinalToggles) {
    ...
    if (param == "CustomPersonalities") {
      ...
      calibrationProgressLabel = new LabelControl(title, QString::number(params.getInt("CalibrationProgress")) + "%", desc);
      ...
      QObject::connect(resetCurveDataBtn, &ButtonControl::clicked, [this]() {
        if (FrogPilotConfirmationDialog::yesorno(tr("Are you sure you want to completely reset your curvature data?"), this)) {
          params.remove("UserLateralAcceleration");
          params_cache.remove("UserLateralAcceleration");
        }
      });
...
void FrogPilotLongitudinalPanel::showEvent(QShowEvent *event) {
  ...
  calibrationProgressLabel->setText(QString::number(params.getInt("CalibrationProgress")) + "%");
...
void FrogPilotLongitudinalPanel::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  parent->keepScreenOn = slcOpen && updatingLimits;
}
